// Superseded by DriverControlled + BlueTeleOp / RedTeleOp, which share one
// implementation and take the alliance from the subclass. Kept for reference and
// commented out so it no longer registers on the Driver Station.
//
...
//import org.firstinspires.ftc.teamcode.opmodes.auto.RedNearWorlds;
//
//@Config
//@TeleOp(name = "Red TeleOp Worlds", group = "1123Competition")
...
//        initPacket.put("x", startPose.get(0, 0));
//        initPacket.put("y", startPose.get(1, 0));
//        initPacket.put("theta", Math.toDegrees(startPose.get(2, 0)));
//        initPacket.put("turret", StaticVariables.turretAngle());
//        initPacket.put("x_real", drivetrain.state.get(0, 0));
//        initPacket.put("y_real", drivetrain.state.get(1, 0));
//        initPacket.put("theta_real", Math.toDegrees(drivetrain.state.get(2, 0)));
//        initPacket.put("turret_real", turret.getAngle());
...
//            // A TelemetryPacket accumulates every canvas op written to it and never
//            // clears, so reusing one across iterations grows the payload without bound.
